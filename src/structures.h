...
struct pluginlink {
	...
	int (*sockgetcharcli)(struct clientparam * cparam, int timeosec, int timeousec);
	int (*sockgetcharsrv)(struct clientparam * cparam, int timeosec, int timeousec);
	int (*sockgetlinebuf)(struct clientparam * cparam, DIRECTION which, unsigned char * buf, int bufsize, int delim, int to);
	int (*myinet_ntop)(int af, void *src, char *dst, socklen_t size);
	int (*dobuf)(struct clientparam * cparam, unsigned char * buf, const unsigned char *s, const unsigned char * doublec);
	int (*dobuf2)(struct clientparam * cparam, unsigned char * buf, const unsigned char *s, const unsigned char * doublec, struct tm* tm, char * format);
	int (*scanaddr)(const unsigned char *s, unsigned long * ip, unsigned long * mask);
	unsigned long (*getip46)(int family, unsigned char *name,  struct sockaddr *sa);
	int (*sockmap)(struct clientparam * cparam, int timeo);
	int (*ACLMatches)(struct ace* acentry, struct clientparam * cparam);
	int (*alwaysauth)(struct clientparam * cparam);
	int (*checkACL)(struct clientparam * cparam);
	...
	void (*freeparam)(struct clientparam * cparam);
	int (*parsehostname)(char *hostname, struct clientparam *cparam, unsigned short port);
	int (*parseusername)(char *username, struct clientparam *cparam, int extpasswd);
	int (*parseconnusername)(char *username, struct clientparam *cparam, int extpasswd, unsigned short port);
	struct sockfuncs *so;
